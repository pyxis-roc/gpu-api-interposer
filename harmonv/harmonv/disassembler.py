...
CUOBJDUMP_SASS_FMT = re.compile(
    r"(\s+/\*(?P<loc>[0-9A-Fa-f]+)\*/\s+(?P<startbrace>{)?\s+(?P<text>.*);\s*(?P<endbrace>})?)?\s+/\*\s+(?P<opcode>0x[0-9A-Fa-f]+)\s+\*/$"
)

SASS_INSN_CUOBJDUMP = namedtuple(
    "SASS_INSN_CUOBJDUMP", "loc opcode text raw vliw_start vliw_end"
)
...
class SASSFunction(object):
    def __init__(
        self, function, sass_disassembly, producer, headers=None, sass_binary=None
    ):
...
class DisassemblerCUObjdump(object):
    ...
    def _parse_cuobjdump_output(src, output):
        ...
        for lno, l in enumerate(output.splitlines(), 1):
            ...
            if m is not None:
                assert (
                    fn_name is None
                ), f"{lno}: Previous function {fn_name} did not end properly"
                ...
                if CUOBJDUMP_RE_FUNC_END.match(l):
                    assert (
                        fn_name is not None
                    ), f"{lno}: End-of-function marker found when no function active"
                    out[fn_name] = fn
                    fn_name = None
                    fn = None
                else:
        ...
        for fn, data in fn_output.items():
            ...
            for lno, l in enumerate(data, 1):
                ...
                if not m:
                    assert (
                        len(disasm) == 0
                    ), f"{lno}: Line '{l}' in middle of disassembly does not match SASS disassembly regular expression"
                    header.append(l)
                    continue
                else:
                    insn = SASS_INSN_CUOBJDUMP(
                        loc=m.group("loc"),
        ...
        for fn, data in fn_output.items():
            ...
            for lno, l in enumerate(data, 1):
                ...
                if m:
                    assert (
                        len(disasm) == 0
                    ), f"{src}:{lno}: Line '{l}' in middle of disassembly looks like a directive, was expecting disassembly"
                    header.append(l)
                    continue
                else:
                    break
    ...
    @staticmethod
    def disassemble(
        cubin, function_names=[], function_index=None, _keep=False, src="<unknown>"
    ):
        ...
        assert not (
            len(function_names) and (function_index is not None)
        ), f"Can't specify both function_names and function_index at the same time"
        ...
        try:
            ...
            for fn, (hdr, sass) in fn_headers_sass.items():
                out[fn] = SASSFunction(
                    fn, sass_disassembly=sass, producer="cuobjdump", headers=hdr
                )
                if fn in fnargs:
                ...
                if fn in syminfo:
                    out[fn].set_syminfo(syminfo[fn])
                if "" in const:
                    out[fn].set_constants(const[""], update=True)
            ...
            logger.error(
                f"ERROR: cuobjdump failed to handle cubin (arch={cubin.arch}): {e}"
            )
            return out
        except:
            raise
